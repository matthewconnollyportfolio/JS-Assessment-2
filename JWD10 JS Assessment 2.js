...
/* ****************************
    TASKS:
      ...
      2. Add an Event listener for the submit button, which will display the score and highlight the correct answers when the button is clicked. Use the code from lines 67 to 86 to help you. **DONE**
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const calculateScore = () => {
    ...
    quizArray.map((quizItem, index) => {
      for (let i = 0; i < 4; i++) {
        ...
        // Radio buttons will stay disabled unless selected by user
        radioElement.disabled = true;

        if (quizItem.a == i) {
          liElement.style.backgroundColor = '#007500' 
        //Green;
        }

        if (radioElement.checked) {
          // Enables whichever radio button is selected
          radioElement.disabled = false;
            if(quizItem.a === i)
              score += 1;
            else
              liElement.style.backgroundColor = '#D10000'         
	    //Red;
        }
      }
    });
  ...
  // Converts to string which allows 00:00 time format
